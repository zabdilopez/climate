...
local_weather %>% 
  ...
  filter(year !=1981 & !(month == 2 & day == 29)) %>% 
  group_by(year) %>% 
  ...
  labs(x=NULL,
       ...
       title = glue("Through {this_month} {this_day}, the cumulative precipitation near Longyearbean, Svalbard is <span style = 'color: dodgerblue'> above average </span> for {this_year}"))+
  theme(
