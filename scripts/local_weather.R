...
my_lat <- 78.21968139431256 *2 *pi/360 # you need to convert the coordinates                                            in radians for it to make sense
my_lon <- 15.63912339134378 *2 *pi/360
...
local_weather <- read_csv(station_daily,
  ...
  mutate(date = ymd(date),
         ...
         PRCP = PRCP /10) %>%
  rename_all(tolower) %>%
  mutate(prcp = if_else(prcp < 60, prcp, NA_real_))%>%
  drop_na(prcp)
